...
Yet, one is actually not concerned with the expression of $$\hat{H}_{mat}$$ since it is not computed in the library and left to the quantum chemistry code calling MolecCav, nor is the action of the matter total dipole moment $$\hat{\mu}_M(R)$$.  
