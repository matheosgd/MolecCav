...
TESTS=("test_algebra" "test_cavity_mode" "test_quantum_ho1d" "test_elem_op" "test_action_total_H_1p1D" "test_construct_total_H_1p1D" "test_normal_modes_1p1D" "test_ND_indexes" "test_mapping")
...
"ut")
  ...
  Claim "$(grep "Number of error(s)" "$OUTPUT_DIR/"test_file_normalztn.log)"
	...
	Claim "$(grep "Number of error(s)" "$OUTPUT_DIR/"test_quantum_ho1d.log)"
	./test_elem_op.exe < ${DATA_DIR}/data_tests.nml > "$OUTPUT_DIR/"test_elem_op.log
	Claim "$(grep "Number of error(s)" "$OUTPUT_DIR/"test_elem_op.log)"
